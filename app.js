/* Mirror OS • Family Christmas Tree
   ...
   - Pause/Play, Next, Shuffle, Voice, Speed, Brightness
   - Offline (service worker)

   Customize:
   - Edit FAMILY array below (names + traits)
   - Edit PHRASE_TEMPLATES and EXTRA_TRAITS pools
*/
...
function pickAffirmation(member){
  ...
  shuffle(uniq);
  const count = 3 + ((Math.random() * 2) | 0); // 3 or 4
...
function setNow(member, line){
  ...
  ui.configPreview.textContent = JSON.stringify(
    { treeMode, shuffleOn, voiceOn, speed: speedKey, brightness, family: FAMILY },
    null,
    2
  );
}
...
function nextMember(){
  ...
  if (idx % order.length === 0) {
    // reshuffle each round if shuffle enabled
    makeOrder();
  }
}
...
function initGold(){
  ...
  const h = baseY - topY;
  const n = Math.floor(900 * clamp((W*H)/900000, 0.8, 1.6)); // scale for screen
  for (let i = 0; i < n; i++){
    const u = Math.random();
    const y = topY + u * h;
...
ui.btnPlay.addEventListener("click", () => {
  ...
  if (!playing) {
    ...
    lastSwitch = performance.now(); // avoids instant skip
  }
  syncButtons();
});
